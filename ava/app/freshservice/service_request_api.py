...
def get_service_request_info(ticket) -> None:
    service_request = check_if_service_request(ticket)

    if service_request is True:
        ...
        try:
            ...
            if response.status_code != 200:
               logs(f"Failed to get service request info from ticket {ticket_id} with HTTP code of {response.status_code}")
                ...
                add_service_request(requested_items, id)
